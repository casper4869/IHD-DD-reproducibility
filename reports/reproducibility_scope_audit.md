...
- The post-submission temporal-precedence audit has separate directional equations, SDI as an exogenous control, unrounded direction-specific BH families, fixed-lag, HC3, and 45-unit exclusion sensitivities, full country-level results, diagnostics, checksums, and a session record. Its classification remains provisional for inferential use because the short series and covariance/diagnostic sensitivity materially affect interpretation. The Newey-West/HC3 contrast is retained as a post-submission internal specification-sensitivity display under `figures/internal_audit_not_for_submission/`; it is not the manuscript Figure 5, which remains the submitted `figures/Figure5.tif` artifact.
...
- The exact submitted-table DD SNP-level run was not retained. This remains a disclosed historical limitation. The archive now also includes a separately labelled targeted current-data sensitivity follow-up for all 29 final traits against both outcomes, while preserving the 15,703-record aggregate discovery manifest.
...
This audit documents the published aggregate-result reconstruction at https://github.com/casper4869/IHD-DD-reproducibility and https://doi.org/10.5281/zenodo.22878656 with the stated limits and rights-retained terms. The audit does not support wording that the historical SNP-level DD run was recovered. It does support reproducibility of the separately labelled final-29-trait targeted sensitivity follow-up; unresolved non-MR provenance remains explicitly disclosed.
