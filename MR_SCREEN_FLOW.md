...
Offline filename, row-index, outcome-ID, method, and aggregate-value checks support the following reconstruction:

- the DD results were generated by the self-developed `find_exposure` code family in its saved DD configuration;
- the IHD results were generated by an earlier IHD-configured version of that same code family;
...
- the alternative Grok script did not generate either result folder: its start index, filename convention, local-PLINK workflow, and fixed T2D outcome do not match the preserved files;
- the `find outcome` script reverses the analytical direction and did not generate these exposure-screen folders.
