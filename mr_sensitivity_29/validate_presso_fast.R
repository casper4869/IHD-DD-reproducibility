...
source(file.path(out,'presso_exact_fast.R'));fast<-make_presso_exact_fast()
logs<-character()
for(k in 1:5){
...
writeLines(logs,file.path(out,'PRESSO_IMPLEMENTATION_VALIDATION.txt'));cat(paste(logs,collapse='\n'),'\n')
